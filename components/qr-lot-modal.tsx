    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-emerald-100 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-extrabold text-base">
                {isEn ? "Digital QR Lot Passport" : "डिजिटल लॉट पासपोर्ट (QR Passport)"}
              </h3>
        ...
        <div className="p-6 space-y-4 text-center">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-2xl p-3.5 text-left text-xs space-y-1.5">
            <div className="flex justify-between">
              <span className="text-gray-500">{isEn ? "Farmer / Origin:" : "किसान का नाम:"}</span>
              <span className="font-bold text-gray-900">{lot.farmerName || "Mukesh Sharma"}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{isEn ? "Crop & Variety:" : "फसल व किस्म:"}</span>
              <span className="font-bold text-gray-900">{lot.cropName} ({lot.variety || "Sharbati"})</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{isEn ? "Quantity:" : "मात्रा:"}</span>
              <span className="font-bold text-emerald-800">{lot.quantityQtl} {isEn ? "Quintals" : "क्विंटल"}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{isEn ? "Quality Grade / Moisture:" : "गुणवत्ता ग्रेड / नमी:"}</span>
              <span className="font-bold text-gray-900">{lot.qualityGrade} • {lot.moisturePercent}% {isEn ? "moisture" : "नमी"}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{isEn ? "Expected Price:" : "अपेक्षित भाव:"}</span>
              <span className="font-bold text-emerald-700">₹{lot.expectedPrice || lot.expectedPricePerQtl}/qtl</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{isEn ? "Location:" : "स्थान:"}</span>
              <span className="font-medium text-gray-800 truncate max-w-[200px]">{lot.farmerLocation || lot.farmLocation}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between gap-2">
          ...
          >
            <Printer className="w-3.5 h-3.5" /> {isEn ? "Print Receipt" : "प्रिंट पर्ची"}
          </button>
          <div className="flex items-center gap-2">
              ...
              onClick={() => {
                if (navigator.share) {
                  navigator.share({
                    ...
                    text: `${lot.cropName} (${lot.quantityQtl} Quintals) available on KrishiSetu.`,
            ...
            >
              <Share2 className="w-3.5 h-3.5" /> {isEn ? "Share" : "शेयर करें"}
            </button>
            <button
              onClick={onClose}
